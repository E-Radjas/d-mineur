...
# Initialize Pygame
pg.init()
fenetre = pg.display.set_mode((300, 300), pg.RESIZABLE)
...
list_im=[case_0,case_1,case_2,case_3,case_4,case_5,case_6,case_7,case_8, mine, case_vide]
...
while running:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            running = False
    if pg.mouse.get_pressed()[0]: decouverte((pg.mouse.get_pos()[0] - 16) // 16, (pg.mouse.get_pos()[1] - 16) // 16)
    for l in range(1, len(matrice) - 1):
        for c in range(1, len(matrice[0]) - 1):
            fenetre.blit(list_im[max(matrice[l][c], 10)], (c * 16, l * 16))
    pg.display.flip()
